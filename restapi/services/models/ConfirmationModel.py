...
class Confirmation(db.Model):
    ...
    def send_email_confirm(self) -> None:
        link = os.getenv('FRONTEND_URL') + url_for('user.confirm',token=self.id)
        MailSmtp.send_email([self.user.email],'Activated User','email.html',link=link,username=self.user.username)
